...
BEGIN_MESSAGE_MAP(CPropertiesWnd, CDockablePane)
	...
	ON_WM_SETFOCUS()
	ON_WM_SETTINGCHANGE()
	ON_COMMAND(ID_EXECUTE_ALL, &CPropertiesWnd::OnExecuteAll)
	ON_COMMAND(ID_EXECUTE_PAUSE, &CPropertiesWnd::OnExecutePause)
	ON_COMMAND(ID_EXECUTE_SKIP, &CPropertiesWnd::OnExecuteSkip)
	ON_COMMAND(ID_GET_EXECUTE_LOCATION, &CPropertiesWnd::OnGetExecuteLocation)
END_MESSAGE_MAP()
...
extern CString server_name;
extern int server_port_id,program_port_id;

void CPropertiesWnd::OnExecuteAll()
{
	Program_controller(server_name,server_port_id,program_port_id).set_execute_state(3);
	OnGetExecuteLocation();
}
void CPropertiesWnd::OnExecutePause()
{
	Program_controller(server_name,server_port_id,program_port_id).set_execute_state(0);
	OnGetExecuteLocation();
}
void CPropertiesWnd::OnExecuteSkip()
{
	Program_controller(server_name,server_port_id,program_port_id).set_execute_state(1);
	OnGetExecuteLocation();
}
void CPropertiesWnd::OnGetExecuteLocation()
{
	if(cp==NULL)
		return;
	int begin_current,begin_line_current,begin_line_position;
	int end_current,end_line_current,end_line_position;

	if(Program_controller(server_name,server_port_id,program_port_id).get_execute_location(
